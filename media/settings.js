...
(function (root) {
  ...
  function providerLogoMarkup(id) {
    ...
    if (!path) return "";
    return `<svg class="provider-logo" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="${path}"></path></svg>`;
  }
  ...
  const ROWS = [
    ...
    },
    {
      ...
      keepOpen: true,
      message: (s) => {
        const provider = providerOf(s, "muse");
        return provider.connected && provider.needsLogin !== true
          ? { type: "logout", provider: "muse" }
