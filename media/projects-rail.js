...
(function () {
  ...
  function makeProviderGlyph(provider, dot, sessionId) {
    const id = provider === "codex" || provider === "claude" || provider === "gemini" || provider === "muse" ? provider : "grok";
    const glyph = document.createElement("span");
    glyph.className = "provider-glyph provider-" + id;
    glyph.title = id === "codex" ? "Codex" : id === "claude" ? "Claude" : id === "gemini" ? "Gemini" : id === "muse" ? "Muse Code" : "Grok";
    glyph.setAttribute("aria-label", glyph.title);
    // Meta's mark draws its loops with holes, so it needs the evenodd rule.
    glyph.innerHTML = `<svg class="provider-logo" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="${PROVIDER_LOGO_PATHS[id]}"></path></svg>`;
