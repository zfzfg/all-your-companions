...
describe("limit offer card (real chat.js in a DOM)", () => {
  it("renders continue / wait / dismiss and marks continue primary on a quota", () => {
    ...
    const card = doc.querySelector(".limit-offer");
    expect(card).not.toBeNull();
    expect(card!.querySelector(".card-title")!.textContent).toBe("Grok usage limit reached");
    const labels = [...card!.querySelectorAll(".cx-card-actions button")].map((b) => b.textContent);
    expect(labels).toEqual(["Continue with Claude", "Wait and try again", "Dismiss"]);
    expect(card!.querySelector(".cx-card-actions button.primary")!.textContent).toBe("Continue with Claude");
  });
  ...
  it("omits continue when no partner is connected and marks wait primary", () => {
    ...
    const labels = [...doc.querySelectorAll(".limit-offer .cx-card-actions button")].map((b) => b.textContent);
    expect(labels).toEqual(["Wait and try again", "Dismiss"]);
    expect(doc.querySelector(".limit-offer .cx-card-actions button.primary")!.textContent).toBe("Wait and try again");
  });

  it("Continue posts limitOfferAnswer with the target and collapses the card", () => {
    const { window, posted, doc } = bootWebview();
    dispatch(window, OFFER);
    const btn = [...doc.querySelectorAll(".limit-offer .cx-card-actions button")]
    ...
    // The title still says what hit the limit; the outcome is the status.
    expect(card.querySelector(".card-title")!.textContent).toBe("Grok usage limit reached");
    expect(card.querySelector(".limit-outcome")!.textContent).toBe("Continued with Claude");
    expect(card.querySelector(".cx-card-actions")).toBeNull();
  });
  ...
  it("Dismiss posts dismiss and a later resolved frame is a no-op", () => {
    ...
    expect(doc.querySelector(".limit-offer")!.classList.contains("resolved")).toBe(true);
  });
  ...
  it("the word rate in a normal agentError does not spawn the card", () => {
    ...
    expect(doc.querySelector(".limit-offer")).toBeNull();
