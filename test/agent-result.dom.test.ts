...
describe("agent result card (real chat.js in a DOM)", () => {
  it("names the role, the model it ran on, and what it cost", () => {
    ...
    const card = doc.querySelector(".card.agent-result");
    ...
    expect(meta).toContain("effort high");
    expect(meta).toContain("42s");
    // The cost is the part that must never be quietly dropped.
    expect(meta).toContain("$1.23 · 4,210 tokens");
  });
  ...
  it("lists touched files and open items, and omits empty sections", () => {
    const { window, doc } = bootWebview();
    dispatch(window, RESULT);
    const titles = [...doc.querySelectorAll(".card.agent-result .agent-result-list-title")]
      .map((el) => el.textContent);
    expect(titles).toEqual(["Files touched (reported) (2)", "Still open (1)"]);
    const files = [...doc.querySelectorAll(".card.agent-result .agent-result-list")][0];
  ...
  it("opens the briefing and the result by run coordinates, never by path", () => {
    ...
    dispatch(window, RESULT);
    const buttons = [...doc.querySelectorAll(".card.agent-result .card-actions button")] as HTMLButtonElement[];
    expect(buttons.map((b) => b.textContent)).toEqual(["Open briefing", "Open result", "Open session"]);
  ...
  it("opens the role's own session", () => {
    const { window, posted, doc } = bootWebview();
    dispatch(window, RESULT);
    const open = [...doc.querySelectorAll(".card.agent-result .card-actions button")]
  ...
  it("omits Open session when the role never got a session id", () => {
    ...
    const labels = [...doc.querySelectorAll(".card.agent-result .card-actions button")].map((b) => b.textContent);
  ...
  it("survives a replay — the card is values only, so it renders the same", () => {
    ...
    const cards = doc.querySelectorAll(".card.agent-result");
...
describe("the card shows the discrepancy, not just the self-report", () => {
  it("labels the role's list as reported and highlights unreported edits", () => {
    ...
    const titles = [...doc.querySelectorAll(".card.agent-result .agent-result-list-title")]
      .map((el) => el.textContent);
    expect(titles).toEqual(["Files touched (reported) (1)", "Edits the role did not report (1)"]);
    // The one row that changes what the next step should be told is the one
    // row that is coloured.
    const flagged = doc.querySelector(".card.agent-result .agent-result-list.unreported");
  ...
  it("shows reported-but-not-observed without flagging it as a warning", () => {
    ...
    expect(doc.querySelector(".card.agent-result .agent-result-list.unreported")).toBeNull();
  });
  ...
  it("prints the same-companion caution when there is one, and nothing when there is not", () => {
    ...
    expect(doc.querySelector(".card.agent-result .agent-result-caution")!.textContent)
      .toContain("same companion");

    const clean = bootWebview();
    dispatch(clean.window, RESULT);
    expect(clean.doc.querySelector(".card.agent-result .agent-result-caution")).toBeNull();
  });
});
